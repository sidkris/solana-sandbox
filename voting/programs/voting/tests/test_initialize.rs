...
fn test_initialize() {
    ...
    let counter_state = voting::state::Counter::try_deserialize(&mut data).unwrap();
    assert_eq!(counter_state.count, 0);
    assert_eq!(counter_state.authority, payer.pubkey());

    let instruction = Instruction::new_with_bytes(
        program_id,
        &voting::instruction::Increment {}.data(),
        voting::accounts::Increment {
            counter,
